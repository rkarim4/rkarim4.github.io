...
function flippage(skip) {
  ...
  setupgame(seed);
/* Version B */  
  $(document).trigger('log', ['PageChange', {seed:seed}]);
/* Version B */ 
...
$(document).on('click', '#clearbutton', function(ev) {
  ...
  commitstate(cleared);
  /* Version B */  
  $(document).trigger('log', ['Clear', cleared]);
/* Version B */ 
...
function setcurnumber(num) {
  ...
  curnumber = num;
/* Version B */  
  $(document).trigger('log', ['numberkey', {seed: currentstate().seed, number: num}]);
/* Version B */ 
}
